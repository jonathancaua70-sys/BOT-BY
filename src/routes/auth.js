...
function validateUsername(username) {
  ...
  // Verifica caracteres permitidos (apenas ASCII para evitar homographs)
  ...
  // Verifica se contém apenas caracteres ASCII após normalização
...
function validateCredentials(username, password) {
  ...
  // Valida username usando função robusta
  const usernameValidation = validateUsername(username);
  if (!usernameValidation.valid) {
    return usernameValidation;
  }
  
  // Valida senha: mínimo 8 caracteres, pelo menos 1 letra maiúscula, 1 minúscula, 1 número
  ...
  // Verifica se a senha contém o username (prática comum evitar)
