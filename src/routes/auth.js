...
// Body: { "key": "XXXX-XXXX-XXXX-XXXX", "username": "...", "password": "..." }
// Usa a key (gerada no Discord /gerarkey) para criar user+senha no menu C++.
  ...
  try {
    ...
    try {
      ...
      const [keyRows] = await connection.query(
        `SELECT id, is_used, is_lifetime, duration_days, creator_avatar
...
router.post('/auth/client-login', applyRateLimit('external'), async (req, res) => {
  ...
  try {
    ...
    if (!boundHwid) {
      ...
      await pool.query(
        'UPDATE users SET hwid = ?, hwid_history = ? WHERE id = ?',
      ...
      if (!history.includes(sanitizedHwid)) {
        history.push(sanitizedHwid);
        await pool.query(
          'UPDATE users SET hwid_history = ? WHERE id = ?',
